...
from app.models import Goal, TrainingPlan, WeeklyPlan, UserTrainingConfig
...
def make_plan(db, user_id):
    def _make(goal):
        p = TrainingPlan(
            id=f"plan_{uuid.uuid4().hex[:12]}", user_id=user_id, goal_id=goal.id,
...
def test_driving_plan_reads_persisted_weekly_plan(db, user_id, today, make_goal, make_plan, training_config):
    ...
    view = generator.plan_week_view(db, user_id, plan, wk)
...
def test_non_driving_plan_never_borrows_the_shared_row(db, user_id, today, make_goal, make_plan, training_config):
    """The regression this module exists for. The wedding drives periodization; the
    marathon plan must NOT surface the wedding's 42-mile target or its frozen flag."""
    make_goal("Wedding", today + timedelta(weeks=6))
    marathon = make_goal("Marathon", today + timedelta(weeks=15))
    plan = make_plan(marathon)
    wk = _monday(today)
    db.add(WeeklyPlan(user_id=user_id, week_start=wk.isoformat(), target_tss=42.0,
                      actual_tss=0.0, is_deload=True, frozen=True))
    db.commit()
    ...
    view = generator.plan_week_view(db, user_id, plan, wk)
    assert view["isPersisted"] is False, "a non-driving plan must replay, not read the shared row"
    assert view["targetMi"] != 42.0
    assert view["frozen"] is False
    assert view["phase"] == "base", "phase must come from the marathon, not the nearer wedding"
...
def test_series_spans_requested_window_and_marks_current(db, user_id, today, make_goal, make_plan, training_config):
    ...
    plan = make_plan(goal)
    weeks = generator.project_week_series(db, user_id, plan, weeks_back=3, weeks_forward=4)
...
def test_future_weeks_have_null_actual_not_zero(db, user_id, today, make_goal, make_plan, training_config):
    ...
    goal = make_goal("Marathon", today + timedelta(weeks=20))
    plan = make_plan(goal)
    weeks = generator.project_week_series(db, user_id, plan, weeks_back=1, weeks_forward=3)
...
def test_actual_comes_from_real_activities(db, user_id, today, make_goal, make_plan,
    ...
    weeks = generator.project_week_series(db, user_id, plan, weeks_back=0, weeks_forward=0)
...
def test_projection_ramps_rather_than_flatlining(db, user_id, today, make_goal, make_plan,
    ...
    weeks = [w for w in generator.project_week_series(db, user_id, plan, weeks_back=0, weeks_forward=5)
             if w["isProjection"]]
    assert len(weeks) == 5
...
def test_weeks_forward_is_capped(db, user_id, today, make_goal, make_plan, training_config):
    ...
    plan = make_plan(goal)
    weeks = generator.project_week_series(db, user_id, plan, weeks_back=0, weeks_forward=9999)
